...
class Channel {
	...
	public function onComplete() {
		completed = true;
		#if !prod
		////@notrace("onComplete " + haxe.Timer.stamp() );
		#end
		//stop();
		if( onEnd!=null ) {
			var cb = onEnd;
...
class ChannelEventInstance extends Channel {//basically a sound instance
	...
	public override function setPlayCursorMs(posMs:Float) {
		...
		if ( res != FMOD_OK){
			#if debug
			////@notrace("[SND][Channel]{"+name+"} Repositionning S err " + FaxeRef.fmodResultToString(res)+" to :"+pos+" ("+posMs+")");
			#end
		}
		
		#if debug
		////@notrace("setPlayCursorMs "+posMs);
		#end
	}
	...
	public override function getVolume() : Float{
		...
		if ( res != FMOD_OK){
			#if debug
			////@notrace("[SND][Channel]{"+name+"} getVolume err " + FaxeRef.fmodResultToString(res));
			#end
		}
		return vol;
	}
	...
	public override function setVolume(v:Float){
		...
		if ( res != FMOD_OK){
			//#if debug
			////@notrace("[SND][Channel]{"+name+"} setVolume err " + FaxeRef.fmodResultToString(res));
			//#end
		}
		else {
			//if ( isDebug ){
				//#if !prod
				////@notrace("[SND][Channel]{"+name+"} setVolume ok " + v);
				//#end
			//}
		}
	}
...
class ChannelLowLevel extends Channel{
	...
	public override function isPlaying(){
		...
		if (data == null) {
			//#if !prod
			////@notrace("no data no playing! "+name);
			//#end
			return false;
		}
		var b : Bool = false;
		...
		if ( res != FMOD_OK ){
			//#if debug
			////@notrace("[SND][ChannelLowLevel]{"+name+"} isPlaying err " + FaxeRef.fmodResultToString(res));
			//#end
			return false;
		}
		return b;
	...
	public override function setPlayCursorMs(posMs:Float) {
		...
		if ( res != FMOD_OK){
			//#if debug
			////@notrace("[SND][Channel]{"+name+"} Repositionning S err " + FaxeRef.fmodResultToString(res)+" to :"+posU+" ("+posMs+")");
			//#end
		}
	}
	...
	public override function setVolume(v:Float){
		if (data == null) {
			//if ( isDebug ){
				//#if !prod
				////@notrace("[SND][Channel]{"+name+"} setVolume no data");
				//#end
			//}
			return;
		}
		...
		if ( res != FMOD_OK){
			...
			if ( isDebug ){
				//#if !prod
				////@notrace("[SND][Channel]{"+name+"} setVolume ok " + v+" corrected:"+vcl);
				//#end
			}
		}
	}
...
class SoundLowLevel extends Sound{
	...
	override function get_length() : Float{
		...
		if ( res != FMOD_OK ){
			#if debug
			////@notrace("impossible to retrieve sound len");
			#end
		}
	...
	public override function play( ?offsetMs : Float = 0.0, ?nbLoops:Int = 1, ?volume:Float = 1.0) : Channel {
		...
		#if debug
		////@notrace("[Sound] offset " + offsetMs);
		////@notrace("play " + haxe.Timer.stamp() );
		#end
...
class Snd {
	...
	public function start(loops:Int=0, vol:Float=1.0, ?startOffsetMs:Float=0.0) {
		if ( DISABLED ) 			{
			//#if debug
			////@notrace("[SND] Disabled");
			//#end
			return;
		}
		if ( sound == null ){
			//#if debug
			////@notrace("[SND] no inner sound");
			//#end
			return;
		}
	...
	public static function dumpMemory(){
		...
		if ( res != FMOD_OK){
			//#if debug
			////@notrace("[SND] cannot fetch snd ram dump ");
			//#end
		}
	...
	public function fadeStop( ?fadeDuration = 100 ) {
		if ( !isPlaying()){
			//#if !prod
			////@notrace("not playing " + name+" winn not unfade");//can cause reentrancy issues
			//#end
			return null;
		}
	...
	public static function release(){
		...
		PLAYING.hardReset();
		released = true;
		////@notrace("releasing fmod");
		Faxe.fmod_release();
		#if !prod
		//@notrace("fmod released");
		#end
	}
	...
	function refresh() {
		if ( curPlay != null ) {
			...
			//#if debug
			////@notrace("[Snd] no playin no refresh "+name);
			//#end
		}
	}
	...
	public function	setPlayCursorSec( pos:Float ) 	{
		if (curPlay != null)	{
			...
			//#if debug
			////@notrace("setPlayCursorSec/no current instance");
			//#end
		}
	}
	...
	public function	setPlayCursorMs( pos:Float ){ 
		if (curPlay != null) 	
			...
			//#if debug
			////@notrace("setPlayCursorMs/no current instance");
			//#end
		}
	}
	...
	static var _stop = function(t:TweenV){
		#if !prod
		//if( t.parent != null )
			////@notrace(t.parent.name+" cbk stopped");
		//else 
			////@notrace(" unbound stop called");
		#end
		t.parent.stop();
	}
	...
	public static function loadSound( path:String, streaming : Bool, blocking : Bool  ) : Sound {
		...
		if( DEBUG_TRACK) //@notrace("Snd:loading " + path);
		...
		if ( res != FMOD_OK){
			#if(!prod)
			//@notrace("unable to load " + path + " code:" + res+" msg:"+FaxeRef.fmodResultToString(res));
			#end
			return null;
		}
	...
	public static function loadEvent( path:String ) : Sound {
		...
		if( DEBUG_TRACK) //@notrace("Snd:loadingEvent " + path);
	...
	public static function loadSingleBank( filename : String ) : Null<faxe.Faxe.FmodStudioBankRef>{
		if ( released ) {
			#if debug 
			//@notrace("FMOD not active "+filename);
			#end
			return null;
		}
		...
		if ( filename.endsWith(".fsb")) {
			#if debug 
			//@notrace("fsb files not supported");
			#end
		...
		////@notrace("trying to load " + filename);
		...
		if (result != FMOD_OK)	{
			#if debug
			//@notrace("FMOD failed to LOAD sound bank with errcode:" + result + " errmsg:" + FaxeRef.fmodResultToString(result) + "\n");
			#end
			return null;
		}
		//else 
		...
		var t1 = haxe.Timer.stamp();
		#if debug
		////@notrace("time to load bank:" + (t1 - t0)+"s");
		#end
		return cast fbank;
	}
}