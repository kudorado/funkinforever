...
class DialogueBoxPsych extends FlxSpriteGroup
{
	...
	public function new(dialogueList:DialogueFile, ?song:String = null)
	{
		...
		box.antialiasing = true; //true; //ClientPrefs.globalAntialiasing;
	...
	function spawnCharacters() {
		...
		for (individualChar in charsMap.keys()) {
			...
			char.updateHitbox();
			char.antialiasing = true; //true; //ClientPrefs.globalAntialiasing;
		...
		if(!dialogueEnded) {
			...
			if(PlayerSettings.player1.controls.ACCEPT) {
				if(!daText.finishedText) {
					...
					daText = new AlphabetPE(DEFAULT_TEXT_X, DEFAULT_TEXT_Y, textToType, false, true, 0.0, 0.7);
					add(daText);
	...
	function startNextDialog():Void
	{
		...
		daText = new AlphabetPE(DEFAULT_TEXT_X, DEFAULT_TEXT_Y, textToType, false, true, curDialogue.speed, 0.7);
		add(daText);
