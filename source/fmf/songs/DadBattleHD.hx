...
class DadBattleHD extends BopeeboHD
{
	...
	function createCrowd()
	{
		...
		crowd.frames = Paths.getSparrowAtlas('bg/week_hd/week1/CROWD_DADBATTLE', 'mods');
		
		crowd.animation.addByPrefix('idle', 'crowd', 18, true);
	...
	public override function getDadIcon(icon:Icon)
	{
		...
		icon.animation.play("dad");icon.loadGraphic(Paths.image('health_icon/week_hd/icons/icon-dad', 'mods'), true, 150, 150);
		icon.animation.add('dad', [0, 1], 0, false, false);
		icon.animation.play("dad");icon.loadGraphic(Paths.image('health_icon/week_hd/icons/icon-dad', 'mods'), true, 150, 150);
		icon.animation.add('dad', [0, 1], 0, false, false);
