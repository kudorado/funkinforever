...
class Senpai extends SongPlayer
{
	...
	override function loadMap()
	{
		...
		gameState.add(bgSchool);

		var bgStreet:FlxSprite = new FlxSprite(repositionShit).loadGraphic(Paths.image('weeb/weebStreet', 'week6'));
		bgStreet.scrollFactor.set(0.95, 0.95);
		gameState.add(bgStreet);

		var fgTrees:FlxSprite = new FlxSprite(repositionShit + 170, 130).loadGraphic(Paths.image('weeb/weebTreesBack', 'week6'));
		fgTrees.scrollFactor.set(0.9, 0.9);
		gameState.add(fgTrees);
		...
		bgGirls.setGraphicSize(Std.int(bgGirls.width * GameState.daPixelZoom));
		bgGirls.updateHitbox();

		if (FlxG.save.data.distractions)
		{
			gameState.add(bgGirls);
		}
	}
	...
	override function createGFAnimationOffsets()
	{
		...
		gf.setGraphicSize(Std.int(gf.width * GameState.daPixelZoom));
		gf.updateHitbox();
	...
	{
		super.ready();
		readySprite.setGraphicSize(Std.int(readySprite.width * GameState.daPixelZoom));
	}

	override function set()
	{
		super.set();
		setSprite.setGraphicSize(Std.int(setSprite.width * GameState.daPixelZoom));
	}

	override function go()
	{
		super.go();
		goSprite.setGraphicSize(Std.int(goSprite.width * GameState.daPixelZoom));
	}
